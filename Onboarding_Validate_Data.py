...
from Onboarding_Configuration_Template import (
    ...
    ConfigurationNotFound, CollectionNotFound
)
    ...
    def get_process_table_from_catalog(self) -> DataFrame:
        ...
        try:
            ...
            SELECT
                ...
                _enrichment_catalog_table.ocr_spatial_info,
        ...
        def safe_wrapper(corpus_document_data: dict, image_path: str) -> pd.Series:
            ...
            try:
                ...
                if field_results is not None:
                    ...
                    should_process = opsiq_audit_flag == "true" or has_validation_errors
...
class BypassDataValidator(Validator):
    """
    No-op validator used when no send_all_to_opsiq flag is set to `bypass`.

    This implementation intentionally skips the validation step and returns
    immediately. It is selected by the factory when the configuration data
    is set to `bypass` sending any of validated/invalidated transactions
    to OpsIQ
    """
    ...
    def validate(self) -> None:
        ...
        Returns:
            None
        """
        log.warning("Skipping Validation Module execution since the send_all_to_opsiq flag is set to `bypass`")
        return
...
class FactoryValidator:
    ...
    def get_configuration(
        ...
        #     return "", {}, False
            ...
            log.warning(f"Collection configuration not found")
            return "", {}, False
        except Exception as e:
            log.error(
    ...
    def get_data_validator(
        ...
        if collection_config.get("send_all_to_opsiq") == "bypass":
            log.info(
                "Configuration data's send_all_to_opsiq is set to `bypass`.\n"
                "Proceeding with the Validator that bypasses this module...\n"
            )
            return BypassDataValidator()
        elif "radix-onboarding-indexed" in zip_key or ("tr-image-extract" in zip_key and has_conf):
            log.info(
