...
class GlueJobExecution(GlueJobBase):
    ...
    def rotate_image(
        ...
        """
        Rotate a grayscale image (90°, 180°, 270°) and append the rotated
        versions to a dictionary of S3-ready image bytes.

        Args:
            imgs_data (dict[str, bytes]): Dictionary mapping S3 keys to image bytes.
            gray_img (Image.Image): Grayscale PIL image to rotate.
            save_format (str): Image format used when saving (e.g., "PNG", "JPEG").
            target_prefix (str): S3 prefix where rotated images will be stored.
            file_name (str): Base filename used to build rotated filenames.

        Returns:
            dict[str, bytes]: Updated dictionary including rotated images.
        """
        ...
        for angle, method in rotations.items():
            rotated = gray_img.transpose(method)
            ...
            target_path = (
                os.path.join(
                    target_prefix, 
                    f"{file_name.rsplit('.', 1)[0]}_rot{angle}.{save_format}"
                ).replace("\\","/")
            )
            imgs_data[target_path] = buffer.getvalue()
        return imgs_data

    @log_execution
    def upload_images(self, imgs_data: dict[str, bytes], bucket_name: str) -> None:
        """
        Upload multiple images to S3 from an in-memory dictionary.
        ...
        Returns:
            None
        """
        for path, img_content in imgs_data.items():
            self.log.info(f"Uploading grayscale: {path}")
    ...
    def image_to_grayscale(self,
        ...
        """
        Convert an image to grayscale, optionally generate rotated versions,
        and upload the resulting images to S3.

        Args:
            file_content (bytes): Raw image file content.
            target_prefix (str): S3 prefix used to derive batch context.
            file_name (str): Original file name.
            bucket_name (str): Target S3 bucket name.
            rotate_images (bool, optional): Whether to generate rotated versions.
                Defaults to True.
        ...
        try:
            ...
            gray_bytes = io.BytesIO() # Save grayscale image to bytes

            save_format = img.format if img.format else "PNG"
            gray_img.save(gray_bytes, format=save_format)

            gray_images_folder = (
                "Onb_Gray_Images" if not rotate_images
                else "Onb_Gray_Images/Rotated"
            )
            batch_name = target_prefix.split("/", 1)[1]
            gray_images_new_target_prefix = gray_images_folder + "/" + batch_name
            gray_file_name = f"{file_name.rsplit('.', 1)[0]}.{save_format}"
            target_key_for_grays = (
                os.path.join(gray_images_new_target_prefix, gray_file_name)
                .replace("\\","/")
            )
            imgs_data = {
                target_key_for_grays: gray_bytes.getvalue()
            }

            if rotate_images:
                imgs_data = GlueJobExecution.rotate_image(
                    imgs_data,
                    gray_img,
                    save_format,
                    gray_images_new_target_prefix,
                    gray_file_name
                )

            self.upload_images(imgs_data, bucket_name)

        except Exception as e:
            self.log.warning(f"Could not convert {file_name} to grayscale: {e}")
        ...
        try:
            ...
            with zipfile.ZipFile(zip_file, "r") as zip_ref:
                ...
                for file_name in file_list:
                    ...
                    self.log.info(f"Uploading original: {target_key}")
                    self.s3_client.put_object(
                        Bucket=bucket_name,
                        Key=target_key,
                        Body=file_content,
                    ...
                    if file_extension in image_extensions:
                        self.image_to_grayscale(file_content, target_prefix, file_name, bucket_name)

            self.log.info(
                f"Successfully extracted {len(file_list)} files to \n"
                f"s3://{bucket_name}/{target_prefix}\n"
                f"Grayscale images saved to s3://{bucket_name}/OnbV2_GrayImages/"
            )
