...
from pyspark.sql.functions import when, col, upper, lit, split, from_json, current_timestamp, date_format, create_map, udf, expr, regexp_extract, substring, element_at, concat_ws, collect_list, first, regexp_replace
...
def get_tr_center_id(item_id, tr_access_token):
    get_item_api = f"https://{tr_base_api}/api/v1.0/Items/{item_id}/GetItem"
    headers = {"Authorization": f"Bearer {tr_access_token}", "Content-Type": "application/json"}
    try:
        res = requests.get(get_item_api, headers=headers)
        if res.status_code == 200:
            return res.json().get('trCenterID')
        else:
            log.error(f"Failed to get item {item_id}. Status code: {res.status_code}, Response: {res.text}")
            return None
    except Exception as e:
        log.error(f"Exception calling GetItem for item {item_id}: {e}")
        return None


def upload_image_to_tr(center_id, item_code, file_name, file_bytes, tr_access_token):
    add_file_api = f"https://{tr_base_api}/api/v1.0/Items/AddFile"
    headers = {"Authorization": f"Bearer {tr_access_token}", "Content-Type": "application/json"}
    payload = {
        "CenterID": center_id,
        "ItemCode": item_code,
    ...
    try:
        res = requests.post(add_file_api, json=payload, headers=headers)
        if res.status_code == 200:
            return "File Uploaded"
        else:
            log.error(f"Failed to upload file {file_name} for item {item_code}. Status: {res.status_code}, Response: {res.text}")
            return f"Error: {res.status_code}"
    except Exception as e:
        log.error(f"Exception calling AddFile for item {item_code}: {e}")
        return f"Exception: {e}"
...
def main(**kwargs):
    ...
    try:
        ...
        if not is_existing_item_batch and config_present:
            ...
            elif bypassed_opsiq_flag:
                total_record_count = str(ocr_catalog_df.count())
            else:
                total_record_count = str(ocr_catalog_df.filter(ocr_catalog_df["data_is_valid"] == "True").count())

            log.info(f"Number of Records in the Batch to Load to TR/Unify: {total_record_count}")
            ...
            if workflow_name.split('-')[-1] == 'retrigger':
                ...
                ocr_df = explode_key_value_pairs(ocr_catalog_df, json_schema, field_list)
            else:
            ...
            log.info(f"Number of Records in the Batch to be updated to TR: {total_record_count}")
